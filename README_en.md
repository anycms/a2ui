...
TypeScript implementation of **[A2UI](https://github.com/a2ui-project/a2ui) v1.0** — a server-driven UI protocol where the server streams component descriptions and data, and a thin client renders them. This repo ships a framework-agnostic core, React and Vue 3 renderers with swappable presets, a framework-agnostic vanilla-DOM renderer, an SSE transport, and live demo galleries. The v1.0 protocol specification is vendored under [`a2ui/specification/`](./a2ui/specification).
...
The six publishable packages are versioned **in lockstep** (one shared version);
`@anycms/a2ui-gallery` and `@anycms/a2ui-vue-gallery` are private and excluded. On release, release-it bumps the
versions (`@release-it/bumper` writes all six `package.json` files), regenerates
